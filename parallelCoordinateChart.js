...
function visualise_parallel_coordinates(id,att_col,atts) {
		  ...
		  d3.csv("https://raw.githubusercontent.com/phuje/smartCityVis/master/data/smartCityData-inhabitants.csv", function(dat) {
			...
			while(i<dat.length) {
				var obj={};
				obj.color=getColorByGesamtwertung(dat[i])
				var j=0;
				while(j<att_info.length) {
					var att=att_info[j];
					if(att.type=='KAT') {
						obj[att.name]=dat[i][att.name];
					} else {
						if(dat[i][att.name]=='') {
							obj[att.name]=0;
						} else {
							obj[att.name]=parseFloat(dat[i][att.name]);
						}
					}
					j++;
				}
			...
			createParallelCoordinates(id,pardat,att_info);
		  });
		...
		function createParallelCoordinates(id,dat,att_info) {
			...
			var dimensions = [];
			var i=0;
			while(i<att_info.length) {
				var cur=att_info[i];
				var dim={};
				dim.key=cur.name;
				if(cur.type=='KAT') {
					dim.type=types['String'];
				} else {
					dim.type=types['Number'];
				}
				dim.description=cur.name;
				dimensions.push(dim);
				i++;
			}
			...
			//var output = d3.select('#'+id).append("pre");
			...
			dimensions.forEach(function(dim) {
				if (!("domain" in dim)) {
					...
					dim.domain = d3_functor(dim.type.extent)(dat.map(function(d) { return d[dim.key]; }));
				}
				if (!("scale" in dim)) {
			...
			ctx.globalAlpha = d3.min([1.15/Math.pow(dat.length,0.3),1]);
			render(dat);
  ...
  var hideInfo = function(d) {
    pcpInfo
      .style("opacity", 0);
			...
			//output.text(d3.tsvFormat(dat.slice(0,24)));
			...
			function brush() {
				...
				var selected = dat.filter(function(d) {
