...
var size_min=99999999;
var size_max=0;
var inh_min=999999999;
var inh_max=0;
...
function initMinMaxVals() {
	var i=0;
	while(i<city_markers.length) {
		if(city_markers[i].size==null) {
			console.log(city_markers[i].name);
		} else {
			// SIZE
			var size=city_markers[i].size;
			...
			city_markers[i].size=size;
		}
		if(city_markers[i].land==null) {
			city_markers[i].land=city_markers[i].name;
		}
		if(city_markers[i].inhabitants==null) {
			console.log(city_markers[i].name);
		} else {
			// INHABITANTS
			var inh=city_markers[i].inhabitants;
			...
			city_markers[i].inhabitants=inh;
		}
		i++;
	}
	...
	d3.csv("https://raw.githubusercontent.com/phuje/Data-test/master/smartCity-score-general.csv",
		function(data) {
			...
			while(i<data.length) {
				var j=0;
				while(j<city_markers.length) {
					if(data[i].Stadt==city_markers[j].name) {
						city_markers[j].Rang=data[i].Rang;
						city_markers[j].Gesamtwertung=data[i].Gesamtwertung;
						city_markers[j].Verwaltung=data[i].Verwaltung;
						city_markers[j]['IT und Kommunikation']=data[i]['IT und Kommunikation'];
						city_markers[j]['Energie und Umwelt']=data[i]['Energie und Umwelt'];
						city_markers[j].Gesellschaft=data[i].Gesellschaft;
						city_markers[j].Mobilität=data[i].Mobilität;
					}
					j++;
				}
				i++;
